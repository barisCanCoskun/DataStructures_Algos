...
// method3
int longestConsecutiveSequence3(const std::vector<int>& nums) {
    if(nums.empty()) return 0;

    std::set<int> mySet{nums.begin(), nums.end()};
    size_t longest{1}, length{1};
    std::set<int>::iterator itNext = mySet.begin();
    std::set<int>::iterator itCurrent = itNext++;

    while(itNext != mySet.end()){
        if(*(itCurrent++) + 1 == *(itNext++)){
            ++length;
            if(length > longest)
                longest = length;
        }
...
int main()
{
    ...
    longestConsecutiveSequence3(vec) << std::endl;
